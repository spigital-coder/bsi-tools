...
function App() {
  ...
  const handleDownloadPDF = async () => {
    ...
    if (!jsPDF || !window.html2canvas) {
      alert("Required libraries are not loaded. Please check your internet connection and refresh.");
    ...
    try {
      // Robust Logo Loader with Timeout
      ...
      const loadLogo = (): Promise<HTMLImageElement | null> => {
        return new Promise((resolve) => {
          ...
          img.src = logoUrl;
          // Set a 5 second timeout for logo loading
      ...
      // Render Capture with html2canvas
      ...
      if (visualizerElement) {
        ...
        doc.addImage(imgData, 'PNG', 15, 75, 180, 80);
      }
      ...
      const specs = [
        ...
        { l: "Max Allowed Pressure", v: `${selectedPart.max_allowable_pressure_psig} psig` },
      ];
      ...
      specs.forEach((spec, i) => {
        ...
        doc.setTextColor(bsDark[0], bsDark[1], bsDark[2]);
        doc.setFont('helvetica', 'normal');
        doc.text(String(spec.v), 100, rowY);
      });
      ...
      doc.setFontSize(7);
      doc.setTextColor(150);
      doc.text('Bellows Systems, Inc. | www.bellows-systems.com | Generated via Digital Configurator', 15, 280);
      
      doc.save(`BSI_Report_${selectedPart.part_number}.pdf`);
    } catch (e) {
      console.error("PDF generation failed:", e);
      alert("Failed to generate PDF. Please check your network and browser settings.");
    } finally {
      setIsSubmitting(false);
    }
  };
